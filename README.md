...
а $S_0$ — цена базового актива, $K$ — страйк, $T$ — время до экспирации, $r$ — безрисковая ставка, $\sigma$ — волатильность, $N(\cdot)$ — стандартная нормальная функция распределения.
...
В качестве зависимой переменной используем **логарифмическую ошибку** между рыночной и теоретической ценой:

$$
bsm\_error\_log = \ln\!\left( \frac{P_{\mathrm{mkt}}}{P_{\mathrm{BSM}}} \right),
$$

где $P_{\mathrm{mkt}}$ — наблюдаемая рыночная цена опциона, $P_{\mathrm{BSM}}$ — теоретическая цена по модели BSM.
...
$$
bsm\_error\_rel = \frac{P_{\mathrm{mkt}} - P_{\mathrm{BSM}}}{P_{\mathrm{BSM}}},
$$

$$
bsm\_error\_raw = P_{\mathrm{mkt}} - P_{\mathrm{BSM}},
$$
...
**Вывод EDA:**  
- сильная связь `log_moneyness` и `iv_rv_gap` (эффект улыбки волатильности);  
- прочие факторы слабо коррелированы, мультиколлинеарность некритична;  
- все признаки обладают ненулевой дисперсией.
...
- **ttm_x_abs_log_moneyness_z** — значимая нелинейная зависимость.  
- **is_call** — подтверждает асимметрию между CALL и PUT.

---

## 8. Выводы

1. Классическая модель Блэка–Шоулза не полностью описывает рынок криптоопционов.  
2. Mispricing объясняется разрывом IV–RV, структурой moneyness и факторами ликвидности.  
3. Результаты подтверждают влияние рыночных фрикций и волатильностных премий.  
4. Реализованный пайплайн можно масштабировать для панельного анализа.  
...
**Заключение:**  
Проект воспроизводимо демонстрирует, как рыночные и волатильностные факторы объясняют отклонения цен опционов от модели Блэка–Шоулза.  
Документ готов для загрузки в репозиторий GitHub и соответствует академическому стилю оформления.
